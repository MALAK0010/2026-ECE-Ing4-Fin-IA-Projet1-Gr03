...
def find_cycles_in_graph(graph: nx.DiGraph, min_length: int = 3) -> List[List[str]]:
    """
    ...
    Example:
        >>> cycles = find_cycles_in_graph(graph, min_length=4)
        >>> print(f"Trouvé {len(cycles)} cycles")
    """
    cycles = []
    
    # Utiliser l'algorithme de Johnson pour trouver les cycles élémentaires
    try:
        for cycle in nx.simple_cycles(graph):
            if len(cycle) >= min_length:
                cycles.append(cycle)
    except nx.NetworkXError:
        # Graphe trop grand, utiliser une approche alternative
        pass
